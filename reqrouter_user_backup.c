...
uint16_t ip_checksum(const void *buf, size_t length) {
...
static bool swap_header(void *data, u64 l) {
    ...
    *dst_addr = eth_tmp;
    ...
    //iph->check = ip_checksum(data, l - offset);
    ...
    //unsigned int des_ip = (unsigned int)iph->daddr;

    //Calculate the new UDP checksum with the whole packet
    //dph->check = udp_checksum(data, l, (in_addr_t)iph->saddr, (in_addr_t)iph->daddr);
...
static void *requestHandler(void *arg) {
    ...
    for (l = 0; l < sp->length; l++) {
        printf("Port in Funktion requestHandler %d\n", sp->ports[l]);
...
//#endif
                ...
                // Swap ETH, IP and UDP header and calculate new checksum
